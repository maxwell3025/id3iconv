...
public class ID3
{
	...
	/**
	 * Read ID3 tag and prepare for retrieval with getXXX Use this method to
	 * reread tag if changed externally
	 * 
	 * @exception NoID3TagException
	 *                If file does not contain an ID3 tag
	...
	public void readTag() throws NoID3TagException, IOException {
		...
		try
		{
			...
			} else
			{
				...
				{
					// tag too short
					// this cannot happen cause we found "TAG" at correct
					// position
	...
	public void removeTag() throws IOException {
		...
		if (checkForTag())
		{
			...
			try
			{
				...
				while ((c = in.read(buf)) > 0 && size > 128)
				{ // truncate the
					// last 128
					// bytes
					if (c + 128 > size)
				...
				{
					System.err.println("Cannot rename " + temp + " to "
					        + mp3_file);
