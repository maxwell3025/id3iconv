 ...
 * 
 */
public class ID3Conv
{
	private static final List<String> NON_UNICODE_FIELDS = Arrays.asList("TDAT", "TIME", "TPOS", "TRCK", "TYER");
	
	private static String encoding = System.getProperty("file.encoding");
	private static boolean isDebug = false;
	private static boolean removev1 = false;
	private static boolean forcev1 = false;
	private static boolean quiet = false;
	private static boolean dry = false;
	private static ID3Conv encoder = new ID3Conv(); // Singleton instance

	private static void processFile(File f)
	{
		// Perform the conversion
		System.out.println("Converting file:" + f.getAbsoluteFile());
		try
		{
	...
	private static void processItem(File it)
	{
		if (it.isDirectory())
		{
			...
			for (File f : list)
			{
				...
				{
					processItem(f);
					System.out.println("Processing directory:" + f.getAbsoluteFile());
				}
				else
				{
					processFile(f);
	...
	private void addFrame(String id, String content) throws Exception
	{
		...
		frame = new ID3v2Frame(id, newbuf2, false, false, false, ID3v2Frame.NO_COMPRESSION, (byte) 0, (byte) 0);
		id3v2.addFrame(frame);
	}

	/**
	 * Convert the ID3 tag from any encoding to Unicode. If the original file contains ID3 v1 tags, it is converted to
	 * ID3v2 in order to support Unicode.
	 * 
	 * @param file
	 */
	private void convert(File file, String srcEncoding) throws Exception
	{
		id3 = new ID3(file); // V1 tag
		id3.encoding = srcEncoding;
		...
		else if (hasv2)
		{
			...
			if (frames != null && frames.size() > 0)
			{
				for (Iterator iter = frames.iterator(); iter.hasNext();)
				{
					...
					{
						// check whether the frame is "numerical string" or URL
						// as defined
						// by IDv2.3. They should be encoded in ISO8859-1, not
						// Unicode
						...
						else
						{
							...
							{
								String s = new String(buf, 1, buf.length - 1, srcEncoding);
								debug(frame.getID() + ": " + s);
								byte[] newbuf = s.getBytes("UnicodeLittle");
								// utf-16LE with leading BOM character, this seems to be the most compatible one
	...
	{ "Blues", "Classic Rock", "Country", "Dance", "Disco", "Funk", "Grunge", "Hip-Hop", "Jazz", "Metal", "New Age",
	        ...
	        "Death Metal", "Pranks", "Soundtrack", "Euro-Techno", "Ambient", "Trip-Hop", "Vocal", "Jazz Funk",
	        ...
	        "A Capella", "Euro-House", "Dance Hall", "Goa", "Drum & Bass", "Club-House", "Hardcore", "Terror", "Indie",
